...
def train_one_epoch(model, optimizer, loss, loader, epoch, device):
    ...
    for step, (batch_ppi_input, batch_img_input, batch_genes) in enumerate(loader):
        latent_ppi, latent_img, logit_scale = model(
            batch_ppi_input, batch_img_input)
        losses = loss(
...
def clip_fit_predict(
    ...
    for epoch in range(config.epochs):
        ...
        losses = train_one_epoch(
            model, optimizer, loss, train_loader, epoch, device)
    ...
    avg = (latent_ppi.detach().cpu().numpy() +
           latent_img.detach().cpu().numpy()) / 2
